...
public class TaskOverlay extends OverlayPanel {
	...
	@Inject
	private TaskOverlay(final BlastFurnacePlugin plugin, final BlastFurnaceConfig config) {
		super(plugin);
	...
	public Dimension render(Graphics2D graphics) {
		...
		int xpGained = client.getSkillExperience(Skill.SMITHING) - plugin.getInitialXpAmount();
		...
		xpComponent.addRow("Per hour:", QuantityFormatter.quantityToStackSize(getPerHourRate(seconds, xpGained)));
		...
		goldComponent.addRow("Per hour:", QuantityFormatter.quantityToStackSize(getPerHourRate(seconds, plugin.getGoldAmount())));
